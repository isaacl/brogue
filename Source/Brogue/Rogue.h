...
#define DCOLS			80
#define DROWS			(ROWS - 2)	// one line at the top for messages;
...
#define LOS_SLOPE_GRANULARITY 16384 // how finely we divide up the squares when calculating slope;
...
#define CHECKMARK_CHAR	0x2713

enum eventTypes {
	KEYSTROKE,
	MOUSE_UP,
	MOUSE_DOWN,
	REPEAT_PREVIOUS_EVENT
};
...
enum tileFlags {
	...
	HAS_MONSTER					= 1 << 3,
	HAS_ITEM					= 1 << 4,
	IN_FIELD_OF_VIEW			= 1 << 5,	// player has unobstructed line of sight
	WAS_VISIBLE					= 1 << 6,
	HAS_DOWN_STAIRS				= 1 << 7,
	HAS_UP_STAIRS				= 1 << 8,
	...
	PERMANENT_TILE_FLAGS = (DISCOVERED | MAGIC_MAPPED | ITEM_DETECTED | DOORWAY),
};
...
#define cellHasTerrainFlag(x, y, flagMask)	(((flagMask) & (tileCatalog[pmap[x][y].layers[DUNGEON]].flags | tileCatalog[pmap[x][y].layers[LIQUID]].flags | tileCatalog[pmap[x][y].layers[SURFACE]].flags | tileCatalog[pmap[x][y].layers[GAS]].flags)) ? true : false)
#define cellHasTerrainType(x, y, terrain)	((pmap[x][y].layers[DUNGEON] == (terrain) || pmap[x][y].layers[LIQUID] == (terrain) || pmap[x][y].layers[SURFACE] == (terrain) || pmap[x][y].layers[GAS] == (terrain)) ? true : false)
...
enum itemFlags {
	...
	ITEM_NO_PICKUP			= 1 << 4,
	ITEM_VORPALIZED			= 1 << 5,
	ITEM_VORPAL_HINTED		= 1 << 6,
	ITEM_VORPAL_IDENTIFIED	= 1 << 7,
...
typedef struct dungeonFeature {
	...
	boolean overrideDungeonFeatures;
...
typedef struct floorTileType {
	...
	enum dungeonFeatureTypes discoverType;	// turns into this when successfully searched if IS_SECRET is set
...
enum terrainFlags {
	...
	OBSTRUCTS_ITEMS					= 1 << 2,		// items can sit on this tile (which obscures the tile graphic)
	...
	OVERRIDES_DUNGEON_LAYER			= 1 << 8,		// automatically sets coincident dungeon tile to floor
	...
	GLOWS							= 1 << 11,		// lights the tile with 25% light from the foreground color
	LAVA_INSTA_DEATH				= 1 << 12,		// kills any non-levitating non-fire-immune creature instantly
	UNKNOWN_TO_MONSTERS				= 1 << 13,		// monsters will not try to avoid the tile no matter its bad effects (UNUSED -- CAN DELETE)
	...
	IS_VOLUMETRIC					= 1 << 21,		// expands and dissipates; only one volumetric tile allowed per cell
	...
	CAUSES_PARALYSIS				= 1 << 26,		// anything caught on this tile are paralyzed until the tile no longer has this flag
	CAUSES_CONFUSION				= 1 << 27,		// causes things on this tile to become confused
	IS_DF_TRAP						= 1 << 28,		// spews gas of type specified in fireType with volume 1000 when stepped on
	IS_NOT_REMEMBERED				= 1 << 29,		// does not persist onscreen when out of sight
	VANISHES_UPON_PROMOTION			= 1 << 30,		// vanishes when randomly creating promotion dungeon feature;
													// can be overwritten regardless depending on priorities and layers of dungeon feature
	CAUSES_EXPLOSIVE_DAMAGE			= 1 << 31,		// is an explosion; deals 15-20 or 50% damage instantly but not again for five turns
	
	PATHING_BLOCKER					= (OBSTRUCTS_PASSABILITY | TRAP_DESCENT | IS_DF_TRAP | LAVA_INSTA_DEATH | IS_DEEP_WATER | IS_FIRE),
	WAYPOINT_BLOCKER				= (OBSTRUCTS_PASSABILITY | TRAP_DESCENT | IS_DF_TRAP | LAVA_INSTA_DEATH | IS_DEEP_WATER),
...
typedef struct statusEffects {
	...
	short nauseous;		// every so often the creature will vomit as well as moving; also suffers a penalty to accuracy and armor
	short immuneToFire;
...
enum hordeFlags {
	HORDE_DISBANDS_ON_LEADER_DEATH	= 1 << 1,	// if the leader dies, the horde will scatter instead of electing new leader
...
enum monsterBehaviorFlags {
	...
	MONST_FLEES_NEAR_DEATH			= 1 << 6,	// monster flees when under 25% health and reengages when over 75%
	...
	MONST_IMMUNE_TO_WEBS			= 1 << 14,	// monster passes freely through webs
		// empty
		// empty
	MONST_NEVER_SLEEPS				= 1 << 17,	// monster is always awake
	MONST_FIERY						= 1 << 18,	// monster carries an aura of flame (but no automatic fire light)
	MONST_INTRINSIC_LIGHT			= 1 << 19,	// monster carries an automatic light of the specified kind
	MONST_IMMUNE_TO_WATER			= 1 << 20,	// monster moves at full speed in deep water and (if player) doesn't drop items
	MONST_RESTRICTED_TO_LIQUID		= 1 << 21,	// monster can move only on tiles that allow submersion
		//empty
	MONST_SUBMERGES					= 1 << 23,	// monster can submerge in appropriate terrain
		// empty
		// empty
		// empty
		// empty
		// empty
		// empty
	MONST_MAINTAINS_DISTANCE		= 1 << 30,	// monster tries to keep a distance of 3 tiles between it and player
	MONST_TELEPORTS					= 1 << 31,	// monster can teleport when fleeing.
...
enum monsterAbilityFlags {
	...
	MA_CAST_FIRE					= 1 << 10,
	MA_CAST_SLOW					= 1 << 11,
	MA_BREATHES_FIRE				= 1 << 12,	// shoots dragonfire at player from a distance
	MA_SHOOTS_WEBS					= 1 << 13,	// monster shoots webs at the player
	MA_ATTACKS_FROM_DISTANCE		= 1 << 14,	// monster shoots from a distance for its attack
	MA_SEIZES						= 1 << 15,	// monster seizes enemies before attacking and cannot attack flying enemies
	
	MAGIC_ATTACK					= (MA_CAST_HEAL | MA_CAST_HASTE | MA_CAST_BLINK | MA_CAST_CANCEL | MA_CAST_SPARK
									   | MA_CAST_FIRE | MA_CAST_SLOW | MA_BREATHES_FIRE | MA_SHOOTS_WEBS | MA_ATTACKS_FROM_DISTANCE),
	SPECIAL_HIT						= (MA_HIT_HALLUCINATE | MA_HIT_STEAL_FLEE | MA_HIT_SAP_STRENGTH | MA_HIT_DEGRADE_ARMOR),
};
...
enum monsterBookkeepingFlags {
	...
	MONST_LEADER					= 1 << 5,	// monster is the leader of a horde
	MONST_FOLLOWER					= 1 << 6,	// monster is a member of a horde
	MONST_CAPTIVE					= 1 << 7,	// monster is tied up
	MONST_SEIZED					= 1 << 8,	// monster is being held
	MONST_SEIZING					= 1 << 9,	// monster is holding another creature immobile
	MONST_SUBMERGED					= 1 << 10,	// monster is currently submerged and hence invisible until it attacks
};
...
typedef struct playerCharacter {
	...
	short foodSpawned;					// number of foods spawned so far this game
	...
	boolean inWater;					// helps with the water light color
...
void fillSpawnMap(enum dungeonLayers layer, enum tileType surfaceTileType, char spawnMap[DCOLS][DROWS]);
void spawnDungeonFeature(short x, short y, dungeonFeature *feat, boolean refreshCell);
void spawnSurfaceEffect(short x, short y, enum dungeonLayers layer, enum tileType surfaceTileType, short startProbability,
...
void updateStatBar(unsigned short statMask);
...
boolean coordinatesAreInMap(short x, short y);
...
void nameOfItem(item *theItem, char *buf, boolean includeSuffix, boolean includeArticle);
