...
// Allocates space, generates a creature of the given type (or a random level-appropriate type if 0),
...
creature *generateMonster(short monsterID) {
	short index, itemChance;
	creature *monst;
	//index = (monsterID ? monsterID : chooseMonster(rogue.depthLevel));
	index = monsterID;
	if (index) {
		monst = (creature *) malloc(sizeof(creature));
		clearStatus(monst);
		monst->info = monsterCatalog[index];
		...
		monst->carriedItem = ((ITEMS_ENABLED && (rogue.depthLevel <= AMULET_LEVEL) && rand_percent(itemChance)) ? generateItem(-1, -1) : NULL);
...
boolean canSeeMonster(creature *monst) {
	...
	if (pmap[monst->xLoc][monst->yLoc].flags & (VISIBLE | CLAIRVOYANT_VISIBLE)
	...
	if (canSeeMonster(monst)) {
		if (player.status.hallucinating) {
			sprintf(buf, "%s%s", (includeArticle ? "the " : ""),
					monsterText[rand_range(1, NUMBER_MONSTER_KINDS - 1)].name);
			return;
		}
		sprintf(buf, "%s%s", (includeArticle ? "the " : ""),
...
boolean spawnMinions(short hordeID, creature *leader) {
	...
	for (iSpecies = 0; iSpecies < theHorde->numberOfMemberTypes; iSpecies++) {
		...
		for (iMember = 0; iMember < count; iMember++) {
			monst = generateMonster(theHorde->memberType[iSpecies]);
...
creature *spawnHorde(short hordeID, short x, short y, long forbiddenFlags) {
	...
	theHorde = &hordeCatalog[hordeID];
	leader = generateMonster(theHorde->leaderType);
	leader->xLoc = x;
	leader->yLoc = y;
...
boolean summonMinions(creature *summoner) {
	...
	char buf[DCOLS];
	char monstName[DCOLS], monstName2[DCOLS];
	...
	for (monst = monsters->nextCreature; monst != NULL; monst = monst->nextCreature) {
		if (monst != summoner && monstersAreTeammates(monst, summoner)) {
			...
			// note the effect this will have if the summoner has normal teammates
	...
	if (seenMinionCount == 0) {
		sprintf(buf, "%s incants darkly!", monstName);
	} else if (seenMinionCount == 1) {
		monsterName(monstName2, minion, false);
		sprintf(buf, "%s incants darkly and a%s %s appears!", monstName, (isVowel(monstName2[0]) ? "n" : ""), monstName2);
	} else {
		sprintf(buf, "%s incants darkly; the air boils as monsters appear!", monstName, monstName2);
	}
...
void teleport(creature *monst) {
	...
	if (monst == &player) {
		...
		monst->yLoc = loc[1];
		pmap[player.xLoc][player.yLoc].flags |= HAS_PLAYER;
		// increase scent turn number so monsters don't sniff at the old cell like idiots
		rogue.scentTurnNumber += 100;
...
void chooseNewWanderDestination(creature *monst) {
	...
	short WPlist[25], WPcoords[25][2], WPcount, currentWP;
	...
	if (monst->creatureState == MONSTER_FLEEING
		&& (WPcount == 0 ||
			(WPcount == 1 && WPcoords[0][0] == monst->comingFrom[0] && WPcoords[0][1] == monst->comingFrom[1]))
		&& (!(pmap[monst->xLoc][monst->yLoc].flags & IN_FIELD_OF_VIEW) || bestMonsterAdvantage < 0)) {
		monst->creatureState = MONSTER_COWERING;
...
boolean monsterAvoids(creature *monst, short x, short y) {
	...
	if (!(monst->status.levitating)) { // monster doesn't fly
		...
		if (!(monst->info.flags & MONST_IMMUNE_TO_FIRE) && cellHasTerrainFlag(x, y, (IS_FIRE | LAVA_INSTA_DEATH))) {
			return true;
		}
		if (!(monst->info.flags & MONST_IMMUNE_TO_WATER) && cellHasTerrainFlag(x, y, IS_DEEP_WATER)) {
			return (!cellHasTerrainFlag(monst->xLoc, monst->yLoc, IS_DEEP_WATER)); // avoid only if not already in it
		}
	}
	return false;
}
...
//	char monstName[COLS], buf[COLS];
	...
	if ((monst->creatureState == MONSTER_WANDERING) && awareOfPlayer) {
		...
		monst->creatureState = MONSTER_TRACKING_SCENT;
	/*} else if (monst->creatureState == MONSTER_WANDERING && (!(monst->bookkeepingFlags & MONST_FOLLOWER) || monst->leader->creatureState == MONSTER_SLEEPING) 
			   && !(monst->info.flags & MONST_NEVER_SLEEPS) && rand_range(1, 500) == 1) {
...
//			}
		...
		chooseNewWanderDestination(monst);
	} else if (monst->creatureState == MONSTER_COWERING) {
		if (pmap[x][y].flags & IN_FIELD_OF_VIEW && distanceBetween(x, y, player.xLoc, player.yLoc) <= 1) {
			monst->creatureState = MONSTER_FLEEING;
			chooseNewWanderDestination(monst); // WEIRDNESS HERE?
		}
	} else if (monst->creatureState == MONSTER_TRACKING_SCENT && monst->info.flags & MONST_MAINTAINS_DISTANCE
...
void decrementMonsterStatus(creature *monst) {
	...
	if (monst->currentHP < monst->info.maxHP && monst->info.turnsBetweenRegen > 0) {
	...
	if (monst->status.burning) {
		...
		if (!(monst->info.flags & MONST_IMMUNE_TO_FIRE) && inflictDamage(NULL, monst, damage)) {
			if (canSeeMonster(monst)) {
...
//	}
	...
	if (monst->info.flags & MONST_SUBMERGES && !(monst->bookkeepingFlags & MONST_SUBMERGED)
		...
		if (rand_percent(20)) {
			monst->bookkeepingFlags |= MONST_SUBMERGED;
			if (!monst->status.magicalFear && (monst->creatureState == MONSTER_FLEEING || monst->creatureState == MONSTER_COWERING)
...
boolean monstUseMagic(creature *monst) {
	...
	float weakestAllyHealthFraction = 1;
	...
	if (!monst->status.confused && monst->info.abilityFlags
		& (MA_CAST_BLINK | MA_CAST_CANCEL | MA_CAST_SLOW | MA_BREATHES_FIRE | MA_SHOOTS_WEBS | MA_ATTACKS_FROM_DISTANCE)) {
		CYCLE_MONSTERS_AND_PLAYERS(target) {
			if (monstersAreEnemies(monst, target) && !(monst->bookkeepingFlags & MONST_SUBMERGED)) {
				if (openPathBetween(monst->xLoc, monst->yLoc, target->xLoc, target->yLoc)) {
					...
					if (monst->info.abilityFlags & MA_BREATHES_FIRE && rand_percent(40)) {
						...
						if (player.currentHP <= 0) {
							gameOver(monsterText[monst->info.monsterID].name, false);
						}
						return true;
					}
					
					// spiders sometimes shoot webs:
					if (monst->info.abilityFlags & MA_SHOOTS_WEBS && rand_percent(20)
					...
					if (monst->info.abilityFlags & MA_ATTACKS_FROM_DISTANCE
						...
						monsterShoots(monst, targetLoc,
									  ...
									  (monst->info.abilityFlags & MA_HIT_DEGRADE_ARMOR ? &green :
										(monst->info.abilityFlags & MA_HIT_SAP_STRENGTH ? &purple : &gray)));
						return true;
					}
					...
					if (monst->info.abilityFlags & (MA_CAST_CANCEL)
						&& (target->status.hasted || target->status.telepathic
							|| ((target->status.immuneToFire || target->status.levitating)
								&& cellHasTerrainFlag(target->xLoc, target->yLoc, (LAVA_INSTA_DEATH | DEEP_WATER | TRAP_DESCENT)))) && rand_percent(50)) {
						if (canSeeMonster(monst)) {
							monsterName(monstName, monst, true);
					...
					if (monst->info.abilityFlags & (MA_CAST_SLOW) && (!target->status.slowed) && rand_percent(50)) {
						if (canSeeMonster(monst)) {
							monsterName(monstName, monst, true);
							sprintf(buf, "%s casts a spell of slowing", monstName);
							combatMessage(buf);
						}
	...
	if (!monst->status.confused && monst->info.abilityFlags & MA_CAST_HASTE && rand_percent(75)) {
		for (target = monsters->nextCreature; target != NULL; target = target->nextCreature) {
			if (target != monst && !target->status.hasted && target->creatureState == MONSTER_TRACKING_SCENT
				...
				targetLoc[0] = target->xLoc;
				targetLoc[1] = target->yLoc;
				zap(originLoc, targetLoc, BOLT_HASTE, 10, false);
				return true;
			}
		}
	}
	
	if (!monst->status.confused && monst->info.abilityFlags & MA_CAST_HEAL && rand_percent(50)) {
		for (target = monsters->nextCreature; target != NULL; target = target->nextCreature) {
			if (target != monst
				&& (target->currentHP / target->info.maxHP < weakestAllyHealthFraction)
				&& monstersAreTeammates(monst, target)
				&& !(monst->bookkeepingFlags & MONST_SUBMERGED)
				&& openPathBetween(monst->xLoc, monst->yLoc, target->xLoc, target->yLoc)) {
				weakestAllyHealthFraction = target->currentHP / target->info.maxHP;
				weakestAlly = target;
			}
		}
		if (weakestAllyHealthFraction < 1) {
			target = weakestAlly;
			targetLoc[0] = target->xLoc;
			targetLoc[1] = target->yLoc;
...
void moveAlly(creature *monst) {
	...
	if (closestMonster && distanceBetween(x, y, player.xLoc, player.yLoc) < 10 && !(monst->info.flags & MONST_MAINTAINS_DISTANCE)) {
		targetLoc[0] = closestMonster->xLoc;
		targetLoc[1] = closestMonster->yLoc;
		moveMonsterPassivelyTowards(monst, targetLoc, false);
	} else if (distanceBetween(x, y, player.xLoc, player.yLoc) < 3 && pmap[x][y].flags & IN_FIELD_OF_VIEW) {
		if (rand_percent(30)) {
...
void monstersTurn(creature *monst) {
	short x, y, roomCenterLoc[2], playerLoc[2], targetLoc[2], dir;
	static boolean failsafe = true;
	char buf[COLS], buf2[COLS];
	creature *ally;
	...
	// inanimate monsters can only use special abilities:
	...
	if ((monst->creatureState == MONSTER_FLEEING || monst->creatureState == MONSTER_COWERING)
	...
	if (monst->creatureState == MONSTER_TRACKING_SCENT) {
	...
	} else if (monst->creatureState == MONSTER_FLEEING) { // HERE IT IS!
		dir = nextStep(safetyMap, monst->xLoc, monst->yLoc);
		if (dir != -1) {
			if (!moveMonster(monst, nbDirs[dir][0], nbDirs[dir][1])) {
				targetLoc[0] = x + nbDirs[dir][0];
				targetLoc[1] = y + nbDirs[dir][1];
		...
		return;
	} else if (monst->creatureState == MONSTER_WANDERING || monst->creatureState == MONSTER_FLEEING) {
		...
		if (monst->bookkeepingFlags & MONST_FOLLOWER && distanceBetween(x, y, monst->leader->xLoc, monst->leader->yLoc) > 2) {
			...
			if (monst->xLoc == monst->destination[0][0] && monst->yLoc == monst->destination[0][1]) {
				...
				if (monst->destination[1][0] > 0) {
					...
					chooseNewWanderDestination(monst);
					if (monst->creatureState == MONSTER_COWERING) {
						if (failsafe) {
							failsafe = false;
							monstersTurn(monst);
							return;
		...
		getRoomCenter(roomCenterLoc, roomContainingCell(x, y));
		// try to step towards the room center; do nothing if it fails
		if (!moveMonsterPassivelyTowards(monst, roomCenterLoc, false)
			&& distanceBetween(x, y, player.xLoc, player.yLoc) <= 1) {
			moveMonster(monst, player.xLoc - x, player.yLoc - y); // attack the player if cornered
		}
...
boolean canPass(creature *mover, creature *blocker) {
	return (!blocker->status.confused && !blocker->status.stuck && !blocker->status.paralyzed
			...
			&& (blocker->leader == mover ||
				(mover->leader != blocker && monstersAreTeammates(mover, blocker)
...
boolean moveMonster(creature *monst, short dx, short dy) {
	...
	if (((defender && (defender->info.flags & MONST_ATTACKABLE_THRU_WALLS))
		...
		&& (!(pmap[newX][newY].flags & (HAS_MONSTER | HAS_PLAYER)) || monst->status.confused || monst->status.entranced ||
			canPass(monst, defender) || monstersAreEnemies(monst, defender))) {
		// if it's a legal move	
		if (pmap[newX][newY].flags & (HAS_PLAYER | HAS_MONSTER)) {
			...
			if (pmap[newX][newY].flags & (VISIBLE | CLAIRVOYANT_VISIBLE)
...
void clearStatus(creature *monst) {
	monst->status.telepathic = monst->status.hallucinating = monst->status.blind = monst->status.levitating
	= monst->status.slowed = monst->status.hasted = monst->status.confused = monst->status.burning
	= monst->status.paralyzed = monst->status.stuck = monst->status.nauseous = monst->status.entersLevelIn
	= monst->status.magicalFear = monst->status.immuneToFire = monst->status.explosionImmunity = monst->status.entranced = 0;
}

// TODO: make generalized version of this function
